...
export default function AdminSettingsPage() {
  ...
  // Role check simulation (Restricted from destructive global operations if standard admin)
  const [currentUserRole, setCurrentUserRole] = useState<"SUPER_ADMIN" | "ADMIN">("SUPER_ADMIN");
    ...
    async function fetchSettings() {
      try {
        ...
        if (data) {
          ...
          setAutoFlagPlagiarism(data.auto_flag_plagiarism ?? true);
          setMaxFileSize(String(data.max_file_size_mb ?? 25));
          setAnnouncementText(data.announcement_banner ?? "");
          setAllowPublicRegistrations(data.allow_public_registrations ?? true);
        }
      } catch (err) {
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Upsert global configuration
      const { error: settingsError } = await supabase
        .from("platform_settings")
        .upsert({
          id: 1,
          maintenance_mode: maintenanceMode,
          auto_flag_plagiarism: autoFlagPlagiarism,
          max_file_size_mb: parseInt(maxFileSize) || 25,
          announcement_banner: announcementText,
          allow_public_registrations: allowPublicRegistrations,
          updated_at: new Date().toISOString(),
        });

      if (settingsError) throw settingsError;

      // 2. Commit immutable record to system audit ledger
      await supabase.from("system_audit_logs").insert({
        admin_id: user?.id || null,
        action: "UPDATE_PLATFORM_SETTINGS",
        details: `Config updated: Maintenance=${maintenanceMode}, MaxSize=${maxFileSize}MB`,
      ...
      setErrorMessage(err.message || "Failed to commit settings changes to database.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="w-full flex flex-col items-center justify-center py-32">
        ...
        <p className="text-xs font-mono text-gray-400">Loading system parameters...</p>
      </div>
    );
  }
