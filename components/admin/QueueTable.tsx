...
import { createClient } from "@/lib/supabase/client"; // Ensure you have your client-side Supabase utility
...
export default function AdminQueuePage() {
  ...
  useEffect(() => {
    async function fetchQueue() {
      ...
      // Fetch documents that need admin attention
      const { data, error } = await supabase
        ...
        .select(`
          ...
          author_email
        `)
        // Filter out approved and rejected notes to keep the queue clean
  ...
  const counts = useMemo(() => {
    ...
    notes.forEach((note) => {
      if (note.reviewed_by) {
        locked++;
      } else if (note.status === "flagged") {
        flagged++;
      } else {
        pending++; // 'pending' or 'changes_requested' without an active reviewer
      }
    });
                ...
                filteredNotes.map((note) => {
                  ...
                  return (
                    <tr key={note.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        {isLocked ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-amber-50 text-amber-700 text-[10px] font-mono font-bold uppercase tracking-wider border border-amber-200/60">
                            <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" /> Locked
                          </span>
                        ) : isFlagged ? (
