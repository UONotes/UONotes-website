      ...
      {password && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
          <div className="flex gap-1 pt-2">
            {[0, 1, 2, 3].map((i) => (
              <span key={i} className={`h-[3px] flex-1 rounded-full transition-colors duration-300 ${i < score ? STRENGTH_COLOR[score] : "bg-gray-200"}`} />
            ))}
          </div>
...
function SignUpFormLogic() {
  ...
  useEffect(() => {
    if (!verified) return;
    // Full reload, not router.replace: the session cookie was set
    // server-side by /verify-signup, and this browser's already-running
    // Supabase client has no way to notice that on its own — reloading
    // forces middleware, the Supabase client, and the navbar to
    // re-initialize against the real, current cookies.
