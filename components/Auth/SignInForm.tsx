...
function SignInFormLogic() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // destination page re-runs middleware/server data against the NEW
      // auth cookie, instead of potentially reusing a cached render from
      // before the user was signed in.
      router.refresh();
...
export function SignInForm() {
  return (
    <Suspense fallback={<div className="w-full max-w-lg h-[480px] rounded-3xl bg-white/50 animate-pulse" />}>
      <SignInFormLogic />
    </Suspense>
  );
}